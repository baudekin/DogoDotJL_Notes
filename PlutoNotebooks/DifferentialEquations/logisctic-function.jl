...
# ╔═╡ 372ec3ef-bd96-44b9-9df7-01d734e9a77d
using DifferentialEquations, Plots, Base.MathConstants, Measures, PlutoUI


# ╔═╡ ba67e124-2de2-4b87-9589-0c8442e89a31
TableOfContents()

# ╔═╡ 07bf5504-59ad-11f0-2de8-8d74985206ff
md"# Verhulst-Pearl Equation"

# ╔═╡ 04c7923d-6fd3-4a80-bf60-cce0d26651d2
...
# ╔═╡ 72fe6060-f68c-40e7-96b4-5d50719f3f65
md"## Problem Description"

# ╔═╡ d07ec6b8-68ee-4122-b33d-3b86d8db717a
md"### Ordinary Differental Equation (ODE)"

# ╔═╡ e43514d2-62de-4220-896b-a284f8bff6d5
md"#### Pearl and Reed's take on Verhulst"

# ╔═╡ db1488c8-eea8-49d1-82ac-29684062459d
md"$\frac{dN}{dt} = rN \left(1 - \frac{N}{K} \right)$"

# ╔═╡ cb5c4ce4-8b68-4036-bc34-395f21da4c5a
md"""
*  $N$ is the unknown function that we are trying to understand expressed at population at any time $t$.
*  $t$ is our time parameter expressed as time span. We should allow this to vary.
*  $r$ is the growth reate of the population. We should allow this to vary.
*  $K$ is the maxium population size the enviroment can support aka saturation/equilibrium point. This is known as equilibrium constant.
"""


# ╔═╡ 74b43488-c9b4-42ef-ac8a-7b7d930a8e7c
md"#### DifferntialEquations.jl"

# ╔═╡ ddcdaa69-7ae3-4071-acf9-6c4ac0380f86
md"$\frac{du}{dt}= up$"

# ╔═╡ 637802b7-ea9a-48ff-97e0-01a178df1edd
md"##### Verhulst-Pearl Equation contains multiple parameters"

# ╔═╡ 50fe64e1-4d60-4edd-b419-ab0334823b42
md"$f(u,p,t)= p[1]u \left(1 - \frac{u}{p[2]} \right)$"

# ╔═╡ fbd1f20f-3de0-4911-b819-9c57d7106a7d
md"""
*  $u$ replaces $N$
*  $p[1]$ replaces $r$
*  $p[2]$ replace $K$
...
# ╔═╡ 0849d882-eb88-430b-b4e6-85e202e94f86
md"## Define Julia Function"

# ╔═╡ 0c86375b-9d67-4ed7-8463-e16378282ab7
# f(u, p, t) = p[1] * u * (1 - u/p[2])

# ╔═╡ c241a8b2-40d9-4d89-bd7a-98a36fe45cf5
md"Clearer way to define the function"

# ╔═╡ c99be5ba-10a7-4b8e-b502-11698f0912f8
...
# ╔═╡ 8c7ab916-00da-4707-a2ae-f950b58eda38
md"## Assign Values for u, p and t"

# ╔═╡ 834c70f9-934e-4dab-8e9b-0059ab46d1a7
md" $u_0$: Population"

# ╔═╡ dd7e8ef4-3244-4aa4-ab50-81c626d8481d
md"Define Parameters"

# ╔═╡ 53be9e64-a3b5-4411-b0fa-e3b2e865153a
md" $r$ is the growth rate per month"

# ╔═╡ 0ff6d2cb-4594-4ca4-b22d-2484ca5dfbd5
md" $K$ Maxium Population"

# ╔═╡ e166e4e4-2afd-469b-8cbb-1fcbc9ab9f0b
md" $t$ is time in months"

# ╔═╡ fb2815a5-0424-4e75-aeba-7198739b9e7d
t₀ = 0.0

# ╔═╡ df74ab95-dc46-4a6d-ac72-f354f142b771
md"## Define Problem"

# ╔═╡ f6516b88-66d2-4232-af88-da579463ad16
md"## Define Solution"

# ╔═╡ e44c17f5-a7c3-4875-b123-00fafb349ef4
md"## Plot Solution"

# ╔═╡ e2cec70b-6fa9-409a-8233-74d45d12bd7a
md"Begining Population: $(@bind N₀ Slider(1.0:20.0, 1.0, true))"

# ╔═╡ 17b4e77d-70cc-41df-80cc-4e292b2f95d8
md"Growth Rate: $(@bind r Slider(0.0:0.01:1.0, 0.14, true))"

# ╔═╡ 99bf5cc7-543e-42b3-84d0-34ddb8610e41
md" K: $(@bind K Slider(100.0:200.0, 200.0, true))"

# ╔═╡ 15b5077a-111c-4593-90a3-5b430bb8219f
p = [r, K]

# ╔═╡ 07c2017d-369a-4b2a-a4b7-7a71de2065c7
md"Months: $(@bind tₙ Slider(1.0:72.0, 72.0, true))"

# ╔═╡ fbd21791-fae6-400a-b29e-610c7f2dbc76
timespan = (t₀, tₙ)

# ╔═╡ 619377f4-6a04-4c7e-8f70-38ad3a3361f8
problem = ODEProblem(verhulst, N₀, timespan, p)

# ╔═╡ a7e1feb9-f840-4a70-a428-61f427f73801
solution = solve(problem)

# ╔═╡ 37b74935-7fc8-4cff-b567-dd1ac46523dd
md"Ending Population: $(round(Int,solution(tₙ)))"

# ╔═╡ 63c8bf26-db77-47ff-90e1-55f5a4144f8d
plot(
	solution,
	legend = false,
	linewidth = 2,
	title = "Verhulst-Pearl Equation",
...
# ╔═╡ 46ed461f-763d-4fcc-9957-5932051e6694
